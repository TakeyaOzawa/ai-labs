...
│   ├── __init__.py
│   ├── models.py                 # ドメインモデル（Step, Executor, StepParams等 — 外部依存なし）
│   ├── ports.py                  # Protocol定義（SlackNotifier, JobRepository等 — 外部依存なし）
│   ├── pipeline_engine.py        # パイプライン実行エンジン（execute_steps, run_pipeline）
│   ├── slack_client.py           # ports.SlackNotifier の実装（subprocess依存）
│   ├── job_manager.py            # ports.JobRepository の実装（ファイルI/O依存）
...
| ディレクトリ | 分類基準 |
|---|---|
| lib/ | 複数スクリプトから共有されるクラス・関数（Pythonパッケージ、Dependency Rule準拠） |
...
| `scripts/lib/models.py` | ドメインモデル（Step, Executor, StepParams等） |
| `scripts/lib/ports.py` | Protocol定義（SlackNotifier, JobRepository等） |
| `scripts/lib/pipeline_engine.py` | パイプライン実行エンジン |
| `scripts/lib/config.py` | 設定・定数の一元管理 |
| `scripts/lib/logger.py` | ログ管理（移動） |
| `scripts/lib/version_check.py` | バージョンチェック（移動） |
| `scripts/lib/slack_client.py` | ports.SlackNotifier の実装 |
| `scripts/lib/gws_client.py` | GWS操作クラス |
| `scripts/lib/job_manager.py` | ports.JobRepository の実装 |
...
| 参照元 | 参照内容 |
|---|---|
| `~/Library/LaunchAgents/com.user.scout-daily-pipeline.plist` | `scripts/run-daily-pipeline.py` |
| `~/Library/LaunchAgents/com.user.scout-weekly-pipeline.plist` | `scripts/run-weekly-pipeline.py` |
...
lib/ 内のモジュール同士は相対importを使用する:

```python
# lib/pipeline_engine.py 内
from .models import Step, ExecutionContext, PipelineConfig
from .ports import SlackNotifier, JobRepository
```

```python
# lib/slack_client.py 内
from .ports import SlackNotifier
```

> ⚠️ lib/ 外のエントリポイントからは `from models import ...`（sys.pathにlib/を追加済み）または `from lib.models import ...`（sys.pathにscripts/を追加済み）のいずれかを統一して使用する。本計画では方式2（sys.pathにlib/を追加）を採用するため `from models import ...` 形式を使用する。
...
from models import PipelineConfig, Step, AgentExecutor
...
from pipeline_engine import *  # noqa: F401, F403
```
...
#### Task 1-2: lib/ports.py — インターフェース定義の集約

- **対象ファイル:** `scripts/lib/ports.py`（新規）
- **変更内容:**
  - 全ての外部依存に対するProtocol定義を1ファイルに集約
  - テスト時のモック定義はこのファイルのProtocolを参照する
  - 外部依存ゼロ（標準ライブラリの `typing.Protocol` のみ使用）

```python
# lib/ports.py — 外部依存ゼロ、テストでのモック定義もここを参照
...
#### Task 1-3: lib/slack_client.py — ports.SlackNotifier の実装
...
- **変更内容:**
  ...
  - `ports.SlackNotifier` Protocolを満たす実装クラス
...
#### Task 1-5: lib/models.py + lib/pipeline_engine.py — 既存モジュールの分割

- **対象ファイル:** `scripts/_pipeline_common.py` → `scripts/lib/models.py` + `scripts/lib/pipeline_engine.py`
- **変更内容:**
  - **models.py**: 既存のdataclass群（Step, Executor, StepParams, PipelineConfig, RetryPolicy等）を集約。外部依存ゼロ。テストで軽量にimport可能
  - **pipeline_engine.py**: 実行エンジン（`execute_steps`, `run_pipeline`, `_execute_step`等）を集約。`ports.py` のProtocolに依存し、具体実装は注入される
  - `ExecutionContext` に依存注入ポイントを追加（`SlackNotifier`, `JobRepository`）
  - 後方互換性のため旧パス `scripts/_pipeline_common.py` に薄いラッパー（re-export）を残す

```python
# lib/models.py — 外部依存ゼロ
from dataclasses import dataclass, field
...
```python
# lib/pipeline_engine.py — ports に依存、具体実装には依存しない
from .models import Step, ExecutionContext, PipelineConfig
from .ports import SlackNotifier, JobRepository
...
#### Task 1-6: lib/gws_client.py — GWS操作の共通化
...
#### Task 2-1: test_models.py

- **対象ファイル:** `scripts/tests/unit/test_models.py`（新規）
- **変更内容:**
  - `Step` / `Executor` / `StepParams` のdataclass生成テスト
  - `RetryPolicy` のデフォルト値・バリデーションテスト
  - `PipelineConfig` の構築テスト
  - dataclass の frozen/immutability テスト

#### Task 2-2: test_pipeline_engine.py

- **対象ファイル:** `scripts/tests/unit/test_pipeline_engine.py`（新規）
- **変更内容:**
  - `build_agent_prompt_with_params()` のプロンプト生成ロジックテスト
  - `execute_steps()` の依存関係解決・リトライロジックテスト（ports をモック）
  - `now_jst()` / `generate_id()` のユーティリティテスト
  - `run_pipeline()` のオプション解析テスト

#### Task 2-3: test_logger.py
...
#### Task 2-4: test_job_manager.py

- **対象ファイル:** `scripts/tests/unit/test_job_manager.py`（新規）
- **変更内容:**
  - ジョブファイル生成のJSON構造テスト
  - `_step_to_job()` の再帰変換テスト
  - `find_child_id()` のツリー探索テスト
  - ジョブステータス遷移テスト

#### Task 2-5: test_slack_client.py
...
#### Task 2-6: test_gws_client.py

- **対象ファイル:** `scripts/tests/unit/test_gws_client.py`（新規）
- **変更内容:**
  - GWS API認証フローのモックテスト
  - ファイル取得・メタデータ抽出のロジックテスト
  - エラーハンドリング（認証失敗、API制限）テスト

#### Task 2-7: test_config.py
...
#### Task 4-2: lib/pipeline_engine.py の参照修正

- **対象ファイル:** `scripts/lib/pipeline_engine.py`
- **変更内容:**
...
- **変更内容:** `from models import ...` / `from pipeline_engine import run_pipeline` 形式に統一（方式2: sys.pathにlib/を追加）
...
| 方式 | メリット | デメリット |
|---|---|---|
| **ドメイン別 + lib/内でDependency Rule（本案）** | 認知負荷低、1機能が1ディレクトリで完結、段階的に厳格化可能 | フルのレイヤー分離ほど厳密ではない |
| **フルのクリーンアーキテクチャ（domain/application/infrastructure/entrypoints）** | 依存方向が完全に制御される、大規模チーム向け | 1人開発では過剰、1機能を追うのに4ディレクトリ横断、ファイル数爆発 |
| **レイヤー分離なし（現状）** | 変更量ゼロ | テスト時のモック困難、依存関係が暗黙的 |
...
## 設計方針: Dependency Rule の適用

### 依存の方向（一方向のみ許可）

```
entrypoints (pipelines/, slack/, jobs/, etc.)
    ↓ import
lib/pipeline_engine.py, lib/slack_client.py, lib/job_manager.py
    ↓ import
lib/ports.py, lib/models.py（外部依存ゼロ）
```

### 原則

1. **models.py と ports.py は外部依存ゼロ** — 標準ライブラリのみ使用。テストで軽量にimport可能
2. **pipeline_engine.py は ports.py のProtocolに依存** — 具体実装（slack_client, job_manager）には依存しない
3. **具体実装（slack_client, job_manager）は ports.py のProtocolを満たす** — 差し替え可能
4. **エントリポイントが依存を組み立てる（Composition Root）** — 具体実装のインスタンス化はエントリポイントで行う

### フルのクリーンアーキテクチャに進む判断基準

以下のいずれかが発生した場合に検討:
- 複数人で開発するようになった
- Slack以外の通知先（Teams, Discord等）が追加された
- パイプライン実行基盤をライブラリとして外部公開したい
- テストのセットアップが複雑になりすぎた
...
- [ ] `from models import Step, PipelineConfig` および `from pipeline_engine import run_pipeline` が成功すること（sys.pathにlib/追加済み環境で）
...
- [ ] 後方互換ラッパー経由での既存import（`from _pipeline_common import run_pipeline`）が動作すること
...
### models.py / ports.py / pipeline_engine.py 分割を選択した理由

- **models.py（外部依存ゼロ）**: テストで `from models import Step` だけimportでき、実行エンジン全体を読み込まなくて済む。将来的にスキーマバリデーション追加時の変更箇所が明確
- **ports.py（外部依存ゼロ）**: Protocol定義を1ファイルに集約することで、テスト時のモック定義が容易。具体実装を知らずにインターフェースだけ参照可能
- **pipeline_engine.py**: 実行ロジックが ports のProtocolにのみ依存し、具体実装（subprocess呼び出し等）に依存しない。Dependency Ruleの適用
- フルのクリーンアーキテクチャ（4層分離）は1人開発・55ファイル規模では認知負荷が高すぎるため、lib/ 内部での軽量なレイヤー分離に留める
...
## Phase 6: 後方互換ラッパー `_pipeline_common.py` の削除

### 概要

Phase 0〜5 完了後、移行期間中に残した後方互換ラッパー `_pipeline_common.py` を削除し、全スクリプトが `lib/` を直接importする形に統一する。

### 現在の `_pipeline_common.py` の役割
...
from pipeline_engine import _notify_slack_reply
from config import load_env
from logger import PipelineLogger, log_error, rotate_log, setup_pipeline_logging
```

### 依存しているファイル（削除時に修正が必要）

| ファイル | importしているシンボル |
|---|---|
| `pipelines/run-daily-pipeline.py` | `HOME, JST, SCRIPTS_DIR, AgentExecutor, CompositeExecutor, OutputParams, PipelineConfig, PipelineContext, ScriptExecutor, SlackParams, Step, StepParams, run_pipeline` |
| `pipelines/run-weekly-pipeline.py` | 同上 + `_notify_slack_reply` |
| `pipelines/run-freshness-pipeline.py` | `SCRIPTS_DIR, PipelineConfig, PipelineContext, ScriptExecutor, Step, run_pipeline` |
| `pipelines/run-poc-planner-pipeline.py` | 同上系 |
| `pipelines/run-github-org-trend-scout-pipeline.py` | 同上系 |
| `pipelines/run-github-repo-analysis-pipeline.py` | `run_ai_command, run_slack_notify, load_env` |
| `pipelines/run-academic-trend-scout-pipeline.py` | `run_ai_command, _notify_slack_reply, run_slack_notify` |
| `pipelines/run-gws-trend-scout-pipeline.py` | `run_ai_command, _notify_slack_reply, run_slack_notify` |
| `slack/dispatch-agent-wrapper.py` | `HOME, SCRIPTS_DIR, AgentExecutor, ExecutionContext, OutputParams, PipelineLogger, SlackParams, Step, StepParams, build_agent_prompt_with_params, load_env, now_jst` |
| `ai/invoke-agent.py` | `InputParams` 等 |

### 削除時の修正内容

各ファイルの `from _pipeline_common import ...` を以下の形式に置換:
...
from models import Step, PipelineConfig, PipelineContext, AgentExecutor, ...
from pipeline_engine import run_pipeline, run_ai_command, run_slack_notify, SCRIPTS_DIR, HOME, JST, ...
from pipeline_engine import _notify_slack_reply  # アンダースコア始まりは明示import
from config import load_env
from logger import PipelineLogger
```

> ⚠️ 各パイプラインスクリプトには既に `sys.path.insert(0, str(Path(__file__).resolve().parent.parent))` が追加済み。
> 削除時はこれを `parent.parent / "lib"` に変更するか、既存の行で `scripts/` がパスに入っているため `lib/` も追加する。
...
| `scripts/_version_check.py` | `lib/version_check.py` に移行済み |
...
| ファイル | 修正内容 |
|---|---|
| `slack/run-slack-dispatch-router.py` | `_pipeline_uses_run_pipeline()` 関数内で `"_pipeline_common" in content` をパイプライン判定に使用している。削除後は `"run_pipeline" in content and "from pipeline_engine" in content` 等に変更する |
| `setup/check-env.py` | `used_by` 文字列リテラル内に `_pipeline_common.py` の記載あり。文字列を更新する |
| `tests/test-dispatch-report-path.py` | 旧テストファイル。pytest移行済みの `tests/unit/test_dispatch_report_path.py` があるため削除可能 |
| `tests/test-rss-source-updater.py` | 同上。`tests/unit/test_rss_source_updater.py` があるため削除可能 |

### 影響範囲まとめ

| 影響 | 詳細 |
|---|---|
| パイプラインスクリプト 8ファイル | import文の書き換え（既にsys.path追加済みなので、import先を変えるだけ） |
| slackスクリプト 1ファイル | import文書き換え + パイプライン判定ロジック修正 |
| aiスクリプト 1ファイル | import文の書き換え |
| テスト | 影響なし（テストはlib/を直接importしている） |
| launchd実行 | 影響なし（sys.pathは各スクリプト冒頭で設定済み） |
| ルートの旧ファイル 3つ | 同時削除（`_pipeline_common.py`, `_version_check.py`, `logger.py`） |

### 削除タイミングの判断基準

以下が **全て** 満たされた時点で削除を実行する:

1. ✅ 全パイプラインが新パスで正常起動確認済み
2. ⬜ launchd経由の定期実行が1回以上成功（daily: 翌朝2:00、weekly: 土曜3:30）
3. ⬜ 全パイプラインのimport文を直接import形式に一括書き換え済み
4. ⬜ 書き換え後に `python3.12 -m pytest tests/ -q` で全テストパス
5. ⬜ 書き換え後に `run-daily-pipeline.py --no-job-file` で手動実行確認

### 実行手順

```bash
# 1. 各パイプラインスクリプトのimport文を直接import形式に書き換え
#    （10ファイル: pipelines/8 + slack/1 + ai/1）

# 2. テスト実行
python3.12 -m pytest ~/scripts/tests/ -q

# 3. 手動実行確認
python3.12 ~/scripts/pipelines/run-daily-pipeline.py --no-job-file 2026-05-18

# 4. 問題なければ削除
rm ~/scripts/_pipeline_common.py
rm ~/scripts/_pipeline_common_backup.py
rm ~/scripts/_version_check.py
rm ~/scripts/logger.py
rm ~/scripts/tests/test-dispatch-report-path.py
rm ~/scripts/tests/test-rss-source-updater.py

# 5. run-slack-dispatch-router.py のパイプライン判定ロジック修正
# 6. setup/check-env.py の文字列リテラル修正
# 7. 最終テスト
python3.12 -m pytest ~/scripts/tests/ -q
```
...
- 削除後に `from _pipeline_common import ...` を使っている未発見のスクリプトがあると即座にImportErrorになる
  - 対策: `grep -rl "_pipeline_common" ~/scripts/ ~/` で事前に全参照を洗い出す
- launchd経由の実行は次回スケジュール時まで検証できない
  - 対策: `manage-scheduler.py reload` 後に手動トリガーで確認
