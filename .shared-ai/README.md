...
| ディレクトリ | 性質 | 内容 | 例 |
|---|---|---|---|
| `rules/` | **行動制約** | 該当時に必ず守るべきルール・コーディング規約（禁止・命令） | dev-environment.md, python-coding-standards.md |
...
| 観点 | rules/ | references/ |
|---|---|---|
| 適用タイミング | 該当時に常に強制（always または fileMatch） | 必要時にreadFile |
...
| `domain-frontmatter.md` | `domain-frontmatter.md` | `rules/` |
| `test-db-guard.md` | `test-db-guard.md` | `rules/` |
| `spec-frontmatter.md` | `spec-frontmatter.md` | `rules/` |
...
3. `~/scripts/setup-symlinks.py` にsymlink定義を追加（Codex rulesの場合）
