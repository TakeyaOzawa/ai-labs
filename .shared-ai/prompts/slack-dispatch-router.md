...
1. 明示的なエージェント名・パイプライン名指定（「tech-trend-scoutで調べて」「dailyを実行して」→ そのまま返す）
2. タスクの種類による判定:
   - **パイプライン実行**（最優先で確認）:
     - 日次パイプライン実行 → `run-daily-pipeline`
     - 週次パイプライン実行 → `run-weekly-pipeline`
     - GitHub org日次トレンド → `run-github-org-trend-scout-pipeline`
     - GWSトレンドスカウト → `run-gws-trend-scout-pipeline`
     - アカデミックトレンドスカウト → `run-academic-trend-scout-pipeline`
