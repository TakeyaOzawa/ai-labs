...
## 共通規約
`readFile: ~/.shared-ai/references/agent-common.md` の §1（前日取得）, §4, §8 に従うこと。
...
対象期間: **基準日の0:00〜23:59（JST）** = 1日分。Notion検索用日付フィルタ: `start_date` = 基準日、`end_date` = 基準日の翌日。
...
8. DBのプロパティ名・ステータス値は実際のスキーマに合わせて判定する
9. 更新者名は必ず表示名に変換する。`user://` 形式のIDをレポートに残さない
10. 概要は具体的な技術要素（テーブル名、API名、サービス名等）を含めて記載する
11. 各アイテムにネクストアクションを明記する
12. 更新内容・ネクストアクションが推定の場合は「（推定）」を付記する
13. 作成者と最終更新者が異なる場合は両方を記載する
