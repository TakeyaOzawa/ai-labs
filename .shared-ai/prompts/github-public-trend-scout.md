...
3. 各リポジトリの情報を収集したら即座にfsAppendで一時ファイル（`Documents/works/scout_histories/github_public_trends/daily/tmp/raw_results.md`）に書き出す（コンテキスト節約）
...
完了後、一時ファイル（`Documents/works/scout_histories/github_public_trends/daily/tmp/raw_results.md`）を削除。
...
7. 出力は日本語で行う
8. PR情報は番号+タイトル+作者のみ記録する（bodyは取得しない）
