...
`scripts/pipelines/run-*-pipeline.py` の実装・編集時に遵守すべきルールと参照情報。
...
│   ├── ports.py                     # Protocol定義
│   ├── pipeline_engine.py           # 実行エンジン + run_pipeline()
│   ├── config.py                    # 設定・定数
│   └── ...
├── pipelines/
...
if job_file:
    # grandchildジョブを更新
    subprocess.run(["python3.12", "scripts/jobs/update-job.py",
...
```bash
python3.12 ~/scripts/jobs/manage-scheduler.py {load|unload|reload|status|list} {label}
```

理由: プラットフォーム差異の吸収 + 将来のクラウドスケジューラ移行に備えた抽象化。

自動実行: `~/Library/LaunchAgents/com.user.{pipeline-name}.plist`
...
| 操作 | コマンド |
|------|---------|
| ジョブ検索 | `python3.12 ~/scripts/jobs/find-job.py` |
| ジョブ更新 | `python3.12 ~/scripts/jobs/update-job.py` |
| ジョブ生成 | `python3.12 ~/scripts/jobs/create-jobs.py` |

ジョブファイルのパスを直接ハードコードしない。`find-job.py` で動的に取得する。

## Slack通知

`NOTIFY_FILE_MAP` でエージェント名→出力ファイルテンプレートを定義。`run_pipeline()` が各ファイルにつき1回 `run_slack_notify()` を呼び出す（`~/scripts/slack/notify-slack.py` を `--thread compact` で実行。H1タイトルが親メッセージ、本文がスレッドにぶら下がる）。
