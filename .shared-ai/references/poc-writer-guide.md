...
`works/poc-something/**/SUMMARY.md` が更新された際のワークフロー定義。

## Step 0: マーカー確認

ファイルの先頭1行目に `<!-- READY_FOR_WRITER -->` マーカーが存在するか確認する。
...
`.shared-ai/prompts/tech-blog-writer.md` をreadFileで読み込み、ワークフローを把握する。
...
完了後、SUMMARY.mdの先頭行から `<!-- READY_FOR_WRITER -->` マーカーを削除する。
