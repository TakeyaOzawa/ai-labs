...
本リポジトリは、複数のAIコーディングツール（Kiro / Claude Code / Codex CLI / .agents）の設定ファイルを一元管理し、ツール間でルール・プロンプト・スキルの一貫性を保つためのdotfiles管理基盤である。
...
├── rules/       ──→ Kiro(Wrapper) / Codex(symlink) / Claude(CLAUDE.md)
