...
```bash
vim ~/.shared-ai/rules/contextual/python-coding-standards.md
```

### 6.2 新しいルールの追加

1. `~/.shared-ai/rules/contextual/` にmdファイルを作成
2. `~/scripts/resolve-shared-ai-rules.py` のRULESリストにパターンとターゲットを追記
3. `~/.shared-ai/rules/always/command-dispatcher.md` のテーブルに追記（コマンド/操作種別に紐づく場合のみ）
