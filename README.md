...
`~/scripts/setup-shared-ai.py` は以下を順番に実行する:
1. `check-env.py`: 環境変数の設定状況チェック
2. `setup-symlinks.py`: 以下のsymlinkを作成する
...
| `~/.codex/rules/*.md` | `~/.shared-ai/rules/*-dispatcher.md` | 個別ファイル |
| `~/.git/hooks/pre-commit` | `~/scripts/git-hooks/pre-commit` | 個別ファイル |
...
| コンテキスト効率 | steeringの `inclusion` タイプを適切に使い分け、不要なコンテキスト消費を抑制する |
...
│   ├── rules/             #   ディスパッチャー + critical/ + quality/
...
.kiro/steering/*.md ─── readFile ───→ .shared-ai/references/
...
.git/hooks/         ─── symlink ────→ scripts/git-hooks/
```
...
```bash
vim ~/.shared-ai/rules/quality/python-coding-standards.md
```

### 6.2 新しいルールの追加

1. `~/.shared-ai/rules/critical/` または `~/.shared-ai/rules/quality/` にmdファイルを作成
2. `~/scripts/resolve-shared-ai-rules.py` のRULESリストにパターンとターゲットを追記
3. `~/.shared-ai/rules/command-dispatcher.md` のテーブルに追記（コマンド/操作種別に紐づく場合のみ）
4. （Kiroのみ・任意）`~/.kiro/steering/` にfileMatch Wrapper_Fileを作成

**CLAUDE.md / GEMINI.md / Codex の個別更新は不要**（dispatcherが自動的に新ルールを参照する）

### 6.3 Steering の inclusion タイプ選択

新しいsteeringを作成する際は、以下の基準で `inclusion` タイプを選択する:

| inclusion | 用途 | 必須フィールド |
|---|---|---|
| `always` | ファイル操作に依存しない常時必要なルール（Slack/Notion連携等） | なし |
| `fileMatch` | 特定ファイル編集時のみ必要なルール（コーディング規約等） | `fileMatchPattern`（複数は配列形式） |
| `auto` | リクエスト内容に応じて自動判断させたいガイド（ワークフロー等） | `name`, `description` |
| `manual` | ユーザーが `#name` で明示指定する低頻度ワークフロー | なし |

**fileMatch の不発対策:** `filematch-dispatcher.md`（`inclusion: always`）がフォールバックとして機能する。`fileMatch` を新規追加した場合は `~/scripts/resolve-shared-ai-rules.py` のRULESリストにも追記すること。

詳細: `~/.shared-ai/references/steering-reference-guide.md`
...
| [issues/completed/shared-ai-knowledge-base.md](issues/completed/shared-ai-knowledge-base.md) | 移行の実装プラン（完了） |

## 10. 変更履歴

| 日付 | 内容 |
|---|---|
| 2026-05-19 | ドキュメント整合性監査。README内リンク切れ修正、symlinkテーブル・依存関係図補完、steering対応表にshared-ai-verify.md追加、scripts/README.md全スクリプト記載、slack-trend-scout-merge agent JSON追加、旧rss-source-updater.py削除、interfaces命名規則例外文書化、audit-documentation.py新規作成 |
| 2026-05-17 | steering inclusion type整備。公式ドキュメント準拠でfileMatchPattern配列化、auto steering にname追加、filematch-dispatcher.md新設（fileMatch不発対策）、steering-reference-guide.mdにinclusion選択基準セクション追加 |
