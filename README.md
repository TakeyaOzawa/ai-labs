...
2. 各ツールの参照設定を追加:
   ...
   - Codex: `ln -s ~/.shared-ai/rules/xxx.md ~/.codex/rules/xxx.md` + `setup-symlinks.py` に定義追加
...
**fileMatch の不発対策:** `filematch-dispatcher.md`（`inclusion: always`）がフォールバックとして機能する。`fileMatch` を新規追加した場合はディスパッチャーのテーブルにも追記すること。
