---
inclusion: manual
description: scoutパイプラインのタスク管理スクリプト（find-task.sh / update-task.sh）のインターフェース仕様。手動でのタスク操作やデバッグ時に参照する
---

# タスク管理スクリプトガイド

scoutパイプライン（日次/週次）のタスク状態を操作するためのスクリプト群。
Watcherが内部で使用するほか、手動でのデバッグ・リカバリにも利用可能。

## スクリプト一覧

| スクリプト | パス | 役割 |
|---|---|---|
| find-task.sh | `~/scripts/find-task.sh` | タスク検索・取得 |
| update-task.sh | `~/scripts/update-task.sh` | タスクステータス更新 |
...
# 特定タスクの状態確認
~/scripts/find-task.sh --pipeline weekly --task-name slack-digest-scout

# 全failedタスクを取得
~/scripts/find-task.sh --pipeline daily --status failed --limit 10
```
...
# 2. 対象タスクをstartingに戻す
TASK_FILE=$(~/scripts/find-task.sh --pipeline daily --status failed --limit 1 | jq -r '.task_file')
TASK_ID=$(~/scripts/find-task.sh --pipeline daily --status failed --limit 1 | jq -r '.tasks[0].task_id')
~/scripts/update-task.sh --task-file "$TASK_FILE" --task-id "$TASK_ID" \
  --set '{"status": "starting", "error": null, "started_at": null, "completed_at": null}'

# 3. 親タスクもrunningに戻す（completedやfailedになっている場合）
~/scripts/update-task.sh --task-file "$TASK_FILE" --scope parent \
...
TASK_FILE=$(~/scripts/find-task.sh --pipeline daily --scope parent | jq -r '.task_file')
