...
入力ファイル: `Documents/works/scout_histories/gws_trends/daily/{YYYY-MM-DD}_gws_daily.md`
...
1. `Documents/works/scout_histories/gws_trends/daily/` 配下の直近7日分を読み込む
...
## 出力
ファイル: `Documents/works/scout_histories/gws_trends/weekly/{YYYY-MM-DD}_gws_weekly_digest.md`
...
5. 同一ドキュメントが複数日に出現する場合は1つに統合
6. 個人メモ・下書き・テンプレートはフィルタで除外
7. 情報なし種別は無理に埋めずスキップ
8. 出力は日本語

## Slack通知
レポート完了後、`mcp_slack_notification_home_slack_post_message` で `channel_id: U076LRL1B35` に投稿。ヘッダー: `📁 GWS週次ダイジェスト: {日付}`。4000文字超はセクション分割。注目ドキュメントTop 5を先頭に配置。Markdown→Slack mrkdwn変換（#→*太字*、[]()→<URL|text>）。投稿失敗はエラー報告のみ（レポート作成は成功扱い）。
