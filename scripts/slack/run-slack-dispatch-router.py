...
def _get_ai_command_builder():
    ...
    spec = spec_from_file_location("ai_cli_utils", SCRIPTS_DIR / "ai" / "ai-cli-utils.py")
...
def _is_pipeline_target(agent_name: str) -> bool:
    """エージェント名が pipelines/ 配下のパイプラインスクリプトを指しているか判定する。"""
    return (SCRIPTS_DIR / "pipelines" / f"{agent_name}.py").exists()
...
def launch_agent(
    ...
    try:
        ...
        if _is_pipeline_target(agent_name):
            script_path = SCRIPTS_DIR / "pipelines" / f"{agent_name}.py"
        ...
        wrapper_cmd = [
            "python3.12", str(SLACK_DIR / "dispatch-agent-wrapper.py"),
            "--agent", agent_name,
            "--thread-ts", thread_ts,
            "--channel", DM_CHANNEL,
