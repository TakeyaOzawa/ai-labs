...
def run_poc_planner(base_date: str) -> None:
    ...
    for material_file in material_files:
        ...
        if run_kiro_cli(prompt, planner_log, agent_name="tech-poc-planner"):
            print(f"[{now_jst()}]       ✅ {material_name} 完了")
            planner_success += 1
        else:
            print(f"[{now_jst()}]       ❌ {material_name} 失敗")
            print(f"[{now_jst()}]       💡 再実行: kiro-cli chat --agent tech-poc-planner --trust-all-tools --no-interactive \"{prompt}\"")
...
def run_freshness_check(base_date: str) -> None:
    ...
    if slack_stale:
        ...
        if run_kiro_cli(prompt, refresh_log, agent_name="slack-user-directory-updater"):
    ...
    if notion_stale:
        ...
        prompt = "ワークフローに従って実行してください。"
        if run_kiro_cli(prompt, refresh_log, agent_name="notion-user-directory-updater"):
