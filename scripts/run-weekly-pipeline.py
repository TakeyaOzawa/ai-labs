...
使い方:
    ...
    python3.12 scripts/run-weekly-pipeline.py --no-task-file
...
def main() -> None:
    ...
    error_log = LOG_DIR / "scout-weekly-pipeline-error.log"
    ...
    for agent in AGENTS:
        ...
        agent_log = LOG_DIR / f"scout-weekly-{agent}.log"
        ...
        if agent == "tech-poc-planner":
            ...
            if use_task_file and child_task_id:
                update_task(task_file, task_id=child_task_id, updates={
    ...
    notify_success = 0
    notify_skipped = 0
    notify_log = LOG_DIR / "scout-weekly-slack-notify.log"
    ...
    if use_task_file and task_file:
        print(f"[{final_now}]    タスクファイル: {task_file}")
...
def create_task_file(base_date: str) -> Path | None:
    """タスクファイルを生成し、パスを返す。"""
    result = subprocess.run(
        ["python3.12", str(SCRIPTS_DIR / "create-weekly-tasks.py"), base_date],
        capture_output=True, text=True,
    )
    if result.returncode != 0:
        return None
