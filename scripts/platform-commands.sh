  ...
  source-env)
    ...
    if [[ -n "$SHELL_RC" ]]; then
      ...
      while IFS= read -r line; do
        ...
        eval "$(grep -E '^export (MY_SLACK_OAUTH_TOKEN|SLACK_REFERENCE_BOT_TOKEN|SLACK_REFERENCE_TEAM_ID|SLACK_DISPATCH_DM_CHANNEL|SLACK_DISPATCH_TARGET_USER|GITHUB_TOKEN|NOTION_TOKEN)=' "$SHELL_RC" 2>/dev/null)" 2>/dev/null
        env | grep -E '^(MY_SLACK_OAUTH_TOKEN|SLACK_REFERENCE_BOT_TOKEN|SLACK_REFERENCE_TEAM_ID|SLACK_DISPATCH_DM_CHANNEL|SLACK_DISPATCH_TARGET_USER|GITHUB_TOKEN|NOTION_TOKEN)='
      ) 2>/dev/null || true
    fi
    ;;
