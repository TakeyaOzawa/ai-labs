...
app = FastAPI(title="TesterAgent", version="1.0.0")
...
async def health_check():
    return {"status": "healthy", "agent": "tester", "role": role_manager.role.name}
...
async def handle_mcp_request(request: MCPRequest) -> MCPResponse:
    try:
        ...
        if request.command == "HEALTH_CHECK":
            return MCPResponse(
                status="success",
                message="TesterAgent is healthy",
...
if __name__ == "__main__":
    ...
    port = int(os.getenv("AGENT_PORT", 8083))
